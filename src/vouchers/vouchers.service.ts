...
export class VouchersService {
    ...
    async create(createVoucherDto: CreateVoucherDto) {
        ...
        const partner = await this.partnersRepository.findOneByOrFail({ id: partnerId })
            .catch(() => { throw new NotFoundException(`Partner with id ${partnerId} not found`) });

        await this.vouchersRepository.create({
            ...voucherData,
            partner
        }).save();
    ...
    async findOne(id: string) {
        return this.vouchersRepository.findOneOrFail({ where: { id }, relations: ['partner'] })
            .catch(() => { throw new NotFoundException(`Voucher with id ${id} not found`) });
    }
    ...
    async update(id: string, updateVoucherDto: UpdateVoucherDto) {
        ...
        if (partnerId) {
            partner = await this.partnersRepository.findOneByOrFail({ id: partnerId })
                .catch(() => { throw new NotFoundException(`Partner with id ${partnerId} not found`) });
        }
