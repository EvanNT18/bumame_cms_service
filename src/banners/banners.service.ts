...
export class BannersService {
    ...
    async create(image: Express.Multer.File, createBannerDto: CreateBannerDto) {
        ...
        const partner = await this.partnersRepository.findOneByOrFail({ id: createBannerDto.partnerId }).catch(() => {
            throw new NotFoundException(`Partner with id ${createBannerDto.partnerId} not found`);
        });
    ...
    async index(options: IPaginationOptions) {
        ...
        const { items, meta, links } = await paginate<Banner>(queryBuilder, options);
    ...
    async findOne(id: string) {
        return this.bannersRepository.findOneOrFail({where: { id }, relations: ['partner']})
            .then(async (banner) => {
                return {
                    ...banner,
                    imageUrl: await this.minioService.getFileUrl(banner.filename),
    ...
    async update(id: string, image: Express.Multer.File, updateBannerDto: UpdateBannerDto) {
        ...
        if (updateBannerDto.partnerId) {
            newPartner = await this.partnersRepository.findOneByOrFail({ id: updateBannerDto.partnerId })
                .catch(() => {
                    throw new NotFoundException(`Partner with id ${updateBannerDto.partnerId} not found`);
                });
        }
